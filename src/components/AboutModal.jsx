...
const SkillTag = ({ text }) => <span className="inline-block px-3 py-1 text-sm bg-gray-800 text-blue-400 rounded-full mr-2 mb-2">#{text}</span>;

// Sound effect for tooltips
const useTooltipSound = () => {
    const soundRef = useRef(null);

    useEffect(() => {
        // Create audio element
        soundRef.current = new Audio("/assets/sci-fi-glitch-sound.mp3"); // Add a blip sound file to your assets
        soundRef.current.volume = 0.5; // Lower volume
        return () => {
    ...
    const playSound = () => {
        if (soundRef.current) {
            ...
            soundRef.current.play().catch((e) => console.log("Audio play error:", e));
        }
    };
...
const IconTooltip = ({ text, color }) => {
    return (
        <motion.div
            initial={{ opacity: 0, y: 10, scale: 0.8 }}
            animate={{ opacity: 1, y: -20, scale: 1, transition: { duration: 0.2 } }}
            exit={{ opacity: 0, y: 10, scale: 0.8 }}
            transition={{
                duration: 0.2,
                ease: [0.175, 0.885, 0.32, 1.275],
            }}
            className="absolute bottom-10 left--0.5 transform -translate-x-1/2 -translate-y-full origin-bottom px-4 py-2 rounded-full text-white text-sm font-medium z-20 min-w-max"
            style={{
                background: `linear-gradient(135deg, ${color}, ${color}dd)`,
                boxShadow: `0 4px 15px -2px ${color}66, 0 0 8px -2px rgba(0, 0, 0, 0.2)`,
                marginTop: "-10px", // Add some margin from the icon
            }}>
            {text}
            <motion.div
                className="absolute -bottom-2 left-1/2 transform -translate-x-1/2 w-4 h-4 origin-top"
                style={{
...
const AboutModal = ({ isOpen, onClose }) => {
    ...
    // Sound effect for tooltips
    const playSound = useTooltipSound();
                    ...
                    <motion.div
                        ...
                        <div className="p-6 md:p-8">
                            <div className="flex justify-between items-start mb-6">
                                ...
                                <button onClick={onClose} className="p-1 rounded-full bg-gray-800 hover:bg-gray-700 transform transition-transform duration-500 hover:rotate-180 active:scale-95">
                                    <X className="h-5 w-5 text-gray-400 transition-colors hover:text-white" />
                                </button>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div>
                                    ...
                                    <div className="flex items-center justify-between mt-14">
                                        ...
                                        <div className="text-center relative group">
                                            <div className="w-12 h-12 mx-auto mb-2 relative group" onMouseEnter={playSound}>
                                                <AnimatePresence>
                                                    <motion.div
                                                        ...
                                                        <IconTooltip text="MongoDB" color="#47A248" />
                                                    </motion.div>
                                                </AnimatePresence>
                                                <svg
                                            ...
                                            <span style={{ color: "#47a248" }} className="font-bold text-2xl">
                                                M
                                            </span>
                                        </div>
                                            ...
                                            <div className="w-12 h-12 mx-auto mb-2 relative group" onMouseEnter={playSound}>
                                                <AnimatePresence>
                                                    <motion.div
                                                        onMouseEnter={playSound}
                                                        className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity">
                                                        <IconTooltip text="Express.js" color="#000000" />
                                                    </motion.div>
                                                </AnimatePresence>
                                        ...
                                        <div className="text-center relative group">
                                            <div className="w-12 h-12 mx-auto mb-2 relative group" onMouseEnter={playSound}>
                                                <AnimatePresence>
                                                    <motion.div
                                                        onMouseEnter={playSound}
                                                        className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity">
                                                        <IconTooltip text="React" color="#61DAFB" />
                                                    </motion.div>
                                                </AnimatePresence>
                                            ...
                                            <span style={{ color: "#61dafb" }} className="font-bold text-2xl">
                                                R
                                            </span>
                                        </div>
                                        ...
                                        <div className="text-center relative group">
                                            <div className="w-12 h-12 mx-auto mb-2 relative group" onMouseEnter={playSound}>
                                                <AnimatePresence>
                                                    <motion.div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity">
                                                        <IconTooltip text="Node.js" color="#8CC84B" />
                                                    </motion.div>
                                                </AnimatePresence>
                                            ...
                                            <span style={{ color: "rgb(140, 200, 75)" }} className="font-bold text-2xl">
                                                N
                                            </span>
                                        </div>
                                    </div>
                                ...
                                <div className="flex justify-center items-center">
                                    <img src="assets/About-me-illustration.png" alt="Developer Illustration" className="max-w-full h-auto" />
                                </div>
                            </div>
                        </div>
                    </motion.div>
